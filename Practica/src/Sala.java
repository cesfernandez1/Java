		...
		try {
			...
			if((sala[fila][columna] == 8) || (sala[fila][columna] == 0 ))
				if(sala[fila][columna] == 8)
					System.out.println("No hay butaca disponible, pertenece al pasillo.");
				else
					System.out.println("Esta butaca no ha sido comprada.");
			else
				sala[fila][columna] = 0;
		}catch(InputMismatchException error) {
			System.out.println("Por favor introduzca solo numeros");
		}catch(ArrayIndexOutOfBoundsException error) {
			System.out.println("Has introudcido numeros erróneos");
		}
	}
